...
def define_constraints(constraint_factory: ConstraintFactory):
    return [
        ...
        no_session(constraint_factory,last_day="THURSDAY"),
    ]

def no_session(constraint_factory : ConstraintFactory, last_day)-> Constraint:
    return (constraint_factory.for_each(Lesson)
                                        .filter(lambda lesson : lesson.timeslot.day_of_week == last_day)
                                        .penalize(HardSoftScore.ONE_SOFT)
                                        .as_constraint("Don't assign ")
                                        )
